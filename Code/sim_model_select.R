...
sim_fit <- function(sim_list, c_grp, grp, rs, family = "gaussian"){
  ...
  ## FD and FN
  d1.5 <- coef(fit1.5, lambda = l1.5)[-1] != 0
  ...
  cpp2 <- sum(c_grp[grp2])
  ## FD and FN
  d2 <- predict(fit2, type = "coefficients", lambda = l2)[-1] != 0
  ...
  cpp3 <- sum(c_grp[grp3])
  ## FD and FN
  d3 <- predict(fit3, type = "coefficients", lambda = l3)[-1] != 0
  ...
  cpp4 <- sum(c_grp[grp4])
  ## FD and FN
  d4 <- predict(fit4, type = "coefficients", lambda = l4)[-1] != 0
  
  # results
  pe <- c("Lasso" = pe1, "CRL" = pe1.5, "GL" = pe2, "CGL" = pe3, "rCGL" = pe4)
  cpp <- c("Lasso" = cpp1,"CRL" = cpp1.5,  "GL" = cpp2, "CGL" = cpp3, "rCGL" = cpp4)
  discoveries <- data.frame("Lasso" = d1, "CRL" = d1.5,  "GL" = d2, "CGL" = d3, "rCGL" = d4)
  
  return(list(pe = pe, cpp = cpp, discoveries = discoveries))
  
}

sim_fit(sim_list = sim_data1[[1]], grp = grp_ind,
...
for(l in 1:length(sim_data1)){
  ...
  sim_disc1[[l]] <- this_fit$discoveries
}
...
for(l in 1:length(sim_data2)){
  ...
  sim_disc2[[l]] <- this_fit$discoveries
}
...
for(l in 1:length(sim_data3)){
  ...
  sim_disc3[[l]] <- this_fit$discoveries
}

save(sim_pe1, sim_pe2, sim_pe3, sim_cpp1, sim_cpp2, sim_cpp3, sim_disc1, sim_disc2, sim_disc3,
